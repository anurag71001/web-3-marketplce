    ...
    <div className='max-w-6xl mx-auto p-2'>
        <nav className='flex justify-between'>
            <div className='flex items-center space-x-2 text-sm'>
                {address ? (<button onClick={disconnect}className='connectWalletBtn'>Hi, {address.slice(0,5)+'...'+address.slice(-3)}</button>) : (<button onClick={connectWithMetamask}className='connectWalletBtn'>Connect your wallet</button>)}
                <p className='headerLink'>Daily deals</p>
                <p className='headerLink'>Help & Support</p>
            </div>
        ...
        <section className='flex items-center space-x-2 py-5'>
            ...
            <div className='flex items-center space-x-2 px-2 md:px-5 py-2 border-black border-2 flex-1'>
                ...
                <input 
                className='flex-1 outline-none' placeholder='Search for Anything' type='text'/>
            </div>
